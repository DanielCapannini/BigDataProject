...
object ProjectJob0{
  ...
  val path_output = "/output/project/job0"

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder.appName("project job 0").getOrCreate()
    ...
    actorsAndActresses.cartesian(rddTitleBasics.filter(_._2 == "movie").flatMap(_._9).filter(x => x!= "\\N").distinct())
